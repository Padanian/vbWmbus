...
class ConsoleClientCliParser {
    ...
    private final IntCliParameter logicalDeviceAddress = new CliParameterBuilder("-ld").setDescription(
    ...
    private final IntCliParameter clientId = new CliParameterBuilder("-cid").setDescription(
    ...
    private final FlagCliParameter useShortNameReferencing = new CliParameterBuilder("-sn")
            .setDescription("Use short name referencing instead of logical name referencing.")
            .buildFlagParameter();
    ...
    private final IntCliParameter encryptionMechanism = new CliParameterBuilder("-enc")
            .setDescription("Encryption mechanism: -1 = none, 0 = AES-GCM-128.")
            .buildIntParameter("enc_mech", -1);
    ...
    private final StringCliParameter password = new CliParameterBuilder("-pass").setDescription(
    ...
    private final StringCliParameter manufacturerId = new CliParameterBuilder("-mid")
            .setDescription("Manufacturer ID of the client.")
            .buildStringParameter("manufacturer_id", "MMM");
    ...
    private final FlagCliParameter useHdlc = new CliParameterBuilder("-hdlc")
            .setDescription("Use HDCL layer instead of wrapper layer.")
            .buildFlagParameter();

    private final FlagCliParameter useUdp = new CliParameterBuilder("-udp")
            .setDescription("Use UDP layer instead of TCP layer.")
            .buildFlagParameter();
    ...
    private final IntCliParameter baudRate = new CliParameterBuilder("-bd")
            ...
            .buildIntParameter("baud_rate", 9600);
    ...
    private final FlagCliParameter enableBaudRateHandShake = new CliParameterBuilder("-eh")
            .setDescription("Enables the baudrate handshake process.")
            .buildFlagParameter();
    ...
    private final IntCliParameter physicalDeviceAddress = new CliParameterBuilder("-pd")
            .setDescription("The HDLC physical device address.")
            .buildIntParameter("physical_dev_address", 0);
    ...
    private void setSecurityLevel(ConnectionBuilder<?> connectionBuilder) throws CliParseException {
        ...
        try {
            authenticationMechanismSelected = AuthenticationMechanism.forId(mechanism);
        } catch (IllegalArgumentException e) {
            throw new CliParseException("Illegal argument for authentication mechanism: " + mechanism);
        }

        byte[] encryptionKeyBytes = null;
        if ((encryptionMechanismSelected == EncryptionMechanism.AES_GCM_128)
                || authenticationMechanismSelected == AuthenticationMechanism.HLS5_GMAC) {
            if (!encryptionKey.isSelected()) {
                if (encryptionMechanismSelected == EncryptionMechanism.AES_GCM_128) {
                    throw new CliParseException("Encryption mechanism \"" + EncryptionMechanism.AES_GCM_128
