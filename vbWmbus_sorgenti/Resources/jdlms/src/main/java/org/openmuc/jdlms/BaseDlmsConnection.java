...
abstract class BaseDlmsConnection implements BaseConnection {
    ...
    synchronized <T extends AxdrType> T send(COSEMpdu pdu) throws IOException {
        ...
        COSEMpdu responsePdu = pollValidResponsePdu();
    ...
    abstract MethodResult authenticateViaHls(byte[] processedChallenge) throws IOException;
    ...
    private COSEMpdu pollValidResponsePdu() throws IOException {
        ...
        else if (settings.referencingMethod() == ReferencingMethod.SHORT) {
            return responsePdu;
        }

        throw new FatalJDlmsException(ExceptionId.ILLEGAL_RESPONSE, Fault.SYSTEM,
                MessageFormat.format("The response {0} was not expected.", responsePdu));
    }
    ...
    private void processInitResponse(HlsSecretProcessor hlsSecretProcessor, byte[] clientToServerChallenge,
        ...
        this.negotiatedFeatures = ConformanceSettingConverter
                .conformanceSettingFor(initiateResponse.negotiatedConformance);
    ...
    private void hls5Connect(HlsSecretProcessor hlsSecretProcessor, byte[] cToSChallange, AAREApdu aare)
            throws IOException {
        ...
        byte[] processedStoC = hlsSecretProcessor.process(serverToClientChallenge, securitySuite, clientSystemTitle,
                this.frameCounter);
        byte[] remoteProcessedCtoS = callHls5Auth(processedStoC);

        int serverFc = ByteBuffer.wrap(remoteProcessedCtoS, 1, 4).getInt();
        byte[] processedCtoS = hlsSecretProcessor.process(cToSChallange, securitySuite, this.serverSystemTitle,
                serverFc);
        if (!Arrays.equals(remoteProcessedCtoS, processedCtoS)) {
    ...
    private byte[] callHls5Auth(byte[] processedChallenge) throws IOException {
        ...
        try {
            remoteResponse = authenticateViaHls(processedChallenge);
            ...
            throw createNewAuthErrorEx(e, e.getAssumedFault());
        } catch (IOException e) {
            throw createNewAuthErrorEx(e, Fault.USER);
        }
    ...
    private static FatalJDlmsException createNewAuthErrorEx(IOException e, Fault assumedFault) {
        return new FatalJDlmsException(ExceptionId.AUTHENTICATION_ERROR, assumedFault,
                "Exception during HLS authentication steps 3 and 4", e);
    }
    ...
    private class SessionLayerListenerImpl implements SessionLayerListener {
        ...
        public void dataReceived(byte[] data, RawMessageDataBuilder rawMessageBuilder) {
            APdu aPdu;
            try {
                aPdu = decodeApdu(data, rawMessageBuilder);
        ...
        private APdu decodeApdu(byte[] data, RawMessageDataBuilder rawMessageBuilder) throws IOException {
            SecuritySuite securitySuite = settings.securitySuite();

            APdu aPdu;
            if (securitySuite.getEncryptionMechanism() != EncryptionMechanism.NONE) {
                aPdu = APdu.decode(data, serverSystemTitle, securitySuite, rawMessageBuilder);
            }
            else {
