...
public class GcmModule {
    ...
    public static byte[] processPlain(byte[] plaintext, int off, int len, byte[] systemTitle, int frameCounter,
            SecuritySuite securitySuite, byte apduTag) throws IOException {
        ...
        return ByteBuffer.allocate(cipheredApduLength)
                .put(apduTag) // COSEM PDU type. e.g. GLO_GET_REQUEST
                .put(lengthBytes)
                .put(controlByte)
    ...
    private static byte securityControlByteFrom(SecuritySuite securitySuite) {
        if (securitySuite.getEncryptionMechanism() != EncryptionMechanism.AES_GCM_128
                || securitySuite.getEncryptionMechanism() != EncryptionMechanism.AES_GCM_256) {
            // TODO error!
        }
        byte sc = 0;
